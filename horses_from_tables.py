...
def parse_file(filename, location):
  ...
  xpath_start = '//div[@id = "innerContent"]/div[1]' if location.get_identity() == 1 else '//body'
  ...
  for i in range(1, race_count + 1):
    ...
    # race metadata
    metadata = tree.xpath(xpath_start_item + '//table[2]//tr[1]//td//text()')
    ...
    for j in range(2, entry_count + 1):
      ...
      entry_datum = tree.xpath(xpath_start_item + '//table[3]//tr[' + str(j) + ']//td/span')
